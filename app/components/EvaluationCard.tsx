...
type EvaluationCardProps = {
  ...
  version?: "v1" | "v2";
  showScoring?: boolean; // 移动端显示评分，桌面端由右侧面板处理
};
...
export default function EvaluationCard({
  ...
  const handleDimensionsChange = (dimensions: DimensionScores) => {
    onUpdate(questionId, answer.modelId, { ...evaluation, dimensions });
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-full">
      ...
      {/* 仅在移动端或显式要求时显示评分区 */}
      {showScoring && (
        <div className="p-2 bg-gray-50 md:hidden">
          {/* 评分 - v1/v2 共用 */}
          <div className="mb-2">
              ...
              <div className="grid grid-cols-3 gap-1">
                {ERROR_CATEGORY_KEYS.map((key) => (
                  <label key={key} className="flex items-start space-x-1 text-xs cursor-pointer">
                    <input
                      type="checkbox"
                      checked={evaluation.cons?.includes(key) || false}
          ...
          {version === "v2" && (
            <DimensionScoreGroup
              value={evaluation.dimensions || DEFAULT_DIMENSIONS}
